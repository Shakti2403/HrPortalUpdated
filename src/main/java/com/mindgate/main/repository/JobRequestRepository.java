...
public class JobRequestRepository implements JobRequestDetailsRepositoryInterface {
	...
	private static final String SELECT_SINGLE_JOB_REQUEST="select * from employee_details where user_id=?";
	@Autowired
	...
	public boolean addNewJobRequest(JobRequest jobrequest) {
		...
		Object[] params = {jobrequest.getProjectDetails().getProjectId(),jobrequest.getEmployeeDetails().getEmployeeId(),jobrequest.getSkill1(),jobrequest.getSkill2(),jobrequest.getSkill3(),jobrequest.getCount()};
	...
	public List<JobRequest> getAllJobRequest() {
		List<JobRequest> allAddresses = jdbcTemplate.query(SELECT_ALL_JOB_REQUEST, new JobRequestRowMapper());
		return allAddresses;
	}
